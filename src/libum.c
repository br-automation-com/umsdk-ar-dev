...
// Forward declaration
static int um_send_msg(um_state *hndl, const int dev, const int cmd,
                        const int argc,  const int *argv,
                        const int argc2, const int *argv2, // optional second subblock
                        const int respc, int *respv);

const char *um_get_version()
{   return LIBUM_VERSION_STR; }
                    ...
                    IPADDR *from, const int timeout)
{
    ...
    if((ret = recvfrom(hndl->socket, (char*) response, response_size, 0,
...
static int um_send(um_state *hndl, const int dev, const unsigned char *data, int dataSize)
{
    ...
    if(hndl->verbose > 1)
    {
        ...
        um_log_print(hndl, 2, __PRETTY_FUNCTION__, "type %d id %d sender %d receiver %d blocks %d options 0x%02X to %s:%d",
        ...
        if(ntohs(header->sub_blocks))
        {
            ...
            um_log_print(hndl, 3, __PRETTY_FUNCTION__, "sub block size %d type %d", data_size, ntohs(sub_block->data_type));
            for(i = 0; i < data_size; i++, data_ptr++)
                um_log_print(hndl, 3, __PRETTY_FUNCTION__, " arg%d: %d (0x%02X)%c", i+1, (int)ntohl(*data_ptr), (int)ntohl(*data_ptr), i<data_size-1?',':' ');
        }
    }
...
static float nm2um(const int nm)
{   return (float)nm/1000.0f; }
...
int um_cmd_ext(um_state *hndl, const int dev, const int cmd, const int argc, const int *argv, int respsize, int *response)
{
...
static int um_update_positions_cache(um_state *hndl, const int sender_id, const int axis_index, const int pos_nm, const int time_step_us)
{
    um_positions *positions = &hndl->last_positions[sender_id];
    int *pos_ptr = NULL;
...
int um_recv_ext(um_state *hndl, um_message *msg, int *ext_data_type, void *ext_data_ptr, const int timeout)
{
    ...
    um_log_print(hndl, 3,__PRETTY_FUNCTION__, "type %d id %d sender %d/%d receiver %d options 0x%02X from %s:%d",
                  type, message_id, sender_id, sender_dev_id, receiver_id, options, inet_ntoa(from.sin_addr), ntohs(from.sin_port));
    ...
    if(sub_blocks > 0 && options & SMCP1_OPT_NOTIFY && is_valid_dev(sender_dev_id))
    {
        ...
        case SMCP1_NOTIFY_STATUS_CHANGED:
            if(data_size > 0 && (data_type == SMCP1_DATA_INT32 || data_type == SMCP1_DATA_UINT32))
            {
                ...
                um_log_print(hndl, 2,__PRETTY_FUNCTION__, "dev %d updated status %d (0x%08X)", sender_id, status, status);
            }
            break;
        case SMCP1_NOTIFY_GOTO_POS_COMPLETED:
            if(data_size > 0 && (data_type == SMCP1_DATA_INT32 || data_type == SMCP1_DATA_UINT32))
            {
                ...
                if(message_id != hndl->drive_status_id[sender_id])
                {
                    ...
                    um_log_print(hndl, 2,__PRETTY_FUNCTION__, "dev %d updated drive status %d msg id %d", sender_id, status, message_id);
                    hndl->drive_status_id[sender_id] = message_id;
                }
                else
                    um_log_print(hndl, 2,__PRETTY_FUNCTION__, "dev %d duplicated drive status %d msg id %d", sender_id, status, message_id);
            }
            break;
        ...
        case SMCP1_NOTIFY_PRESSURE_CHANGED:
            if(data_size > 0 && (data_type == SMCP1_DATA_INT32 || data_type == SMCP1_DATA_UINT32))
                ...
                status = -1;
            um_log_print(hndl, 2,__PRETTY_FUNCTION__,"Pressure changed notification from %d/%d, %d channel%s, valves 0x%02x", sender_id, sender_dev_id,
                          data_size-1, data_size-1>1?"s":"", status);
            break;
        default:
    ...
    if(sub_blocks > 1 && ext_data_type != NULL && *ext_data_type >= 0)
    {
        ...
        um_log_print(hndl, 2,__PRETTY_FUNCTION__,"ext data type %d, %d item%s", *ext_data_type, data_size2, data_size2>1?"s":"");
    ...
    if(options&SMCP1_OPT_ACK)
    {
        ...
        um_log_print(hndl, 2,__PRETTY_FUNCTION__, "ACK to %d id %d while %d expected", type, message_id, hndl->message_id);
        return 0;
    }
    ...
    if(!(options&SMCP1_OPT_REQ))
    {
        ...
        um_log_print(hndl, 2,__PRETTY_FUNCTION__, "response to %d id %d while %d expected", type, message_id, hndl->message_id);
        return 0;
    }
                        ...
                        const int respc, int *respv)
{
    ...
    if(dev != SMCP1_ALL && dev != SMCP1_ALL_DEVICES &&
    ...
    for(i = 0; i < (ack_requested?hndl->retransmit_count:1); i++)
    {
        ...
        while((ret = um_recv(hndl, &resp)) >= 0 ||
              ((ret == LIBUM_TIMEOUT || ret == LIBUM_INVALID_DEV) &&
               (int)get_elapsed(start) < hndl->timeout))
        {
            um_log_print(hndl, 4,__PRETTY_FUNCTION__, "ret %d %dms left", ret, (int)(hndl->timeout-get_elapsed(start)));
            if(ret == 1)
                ack_received = true;
            ...
            if((respc || resp_option_requested) && ret == 2)
            {
                ...
                um_log_print(hndl, 3,__PRETTY_FUNCTION__, "%d data item%s of type %d",
...
int um_take_step(um_state *hndl, const int dev,
                 const float step_x, const float step_y, const float step_z, const float step_w,
                 const int spd_x, const int spd_y, const int spd_z, const int spd_w, const int mode, const int max_acc)
{
    int args[10], argc = 0;
...
int um_read_version(um_state * hndl, const int dev,
...
int um_get_positions(um_state *hndl, const int dev, const int time_limit,
...
int ums_get_bowl_control(um_state *hndl, const int dev, ums_bowl_control *control, ums_bowl_center *centers)
{
    ...
    if((ret = um_send_msg(hndl, dev, SMCP1_CMD_UMS_GET_BOWL_CONTROL, 0, NULL, 0, NULL, UMS_BOWL_CONTROL_HEADER_SIZE+UMS_BOWL_MAX_COUNT, resp)) < 0)
        return ret;
